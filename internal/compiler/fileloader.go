...
func processAllProgramFiles(
	...
	if len(loader.outputDtsToSource) == 0 {
		loader.resolver = module.NewResolver(host, compilerOptions)
	} else {
		loader.resolver = module.NewResolver(&ReferenceResolutionHost{
			host:              host,
...
func (p *fileLoader) addProjectReferenceTasks(projectReferences []*core.ProjectReference, hasRootFiles bool) {
	...
	p.configToProjectReference = make(map[tspath.Path]*tsoptions.ParsedCommandLine)
	p.referencesInConfigFile = make(map[tspath.Path][]*tsoptions.ParsedCommandLine)
	...
	if hasRootFiles {
		for _, resolved := range p.resolvedProjectReferences {
			...
			if p.programOptions.UseSourceOfProjectReference && !p.compilerOptions.DisableSourceOfProjectReferenceRedirect.IsTrue() {
				...
				// !!! sheetal check if ModuleKindNone is possible - right now it doesnt look like possible
...
func (p *fileLoader) collectProjectReferences(
	...
	result := make([]*tsoptions.ParsedCommandLine, len(tasks))
	for i, task := range tasks {
		...
		// !!! sheetal this is source file comparison
		if p.compilerOptions.ConfigFilePath != task.resolved.ConfigName() {
			...
			if p.programOptions.UseSourceOfProjectReference && !p.compilerOptions.DisableSourceOfProjectReferenceRedirect.IsTrue() {
				if outFile := task.resolved.CompilerOptions().OutFile; outFile == "" {
					declDir := task.resolved.CompilerOptions().DeclarationDir
					if declDir == "" {
						declDir = task.resolved.CompilerOptions().OutDir
					}
					if declDir != "" {
						p.dtsDirectories.Add(tspath.ToPath(declDir, p.host.GetCurrentDirectory(), p.host.FS().UseCaseSensitiveFileNames()))
					}
				} else {
					p.dtsDirectories.Add(tspath.ToPath(tspath.GetDirectoryPath(outFile), p.host.GetCurrentDirectory(), p.host.FS().UseCaseSensitiveFileNames()))
				}
			}
		}
...
func (p *fileLoader) parseSourceFile(t *parseTask) (*ast.SourceFile, *tsoptions.ParsedCommandLine) {
	...
	t.path = path
	var redirect *tsoptions.ParsedCommandLine = nil

	if p.programOptions.UseSourceOfProjectReference && !p.compilerOptions.DisableSourceOfProjectReferenceRedirect.IsTrue() {
		// Map to source file from project reference
		source, ok := p.outputDtsToSource[path]
		if !ok &&
			p.compilerOptions.PreserveSymlinks.IsTrue() &&
			...
			realpath := tspath.ToPath(p.resolver.GetHost().FS().Realpath(fileName), p.host.GetCurrentDirectory(), p.host.FS().UseCaseSensitiveFileNames())
			if realpath != path {
				source, ok = p.outputDtsToSource[realpath]
			}
		}
		if ok {
			if source.Source != "" {
